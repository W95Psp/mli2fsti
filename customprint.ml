...
and core_type1 ctxt f x =
  if x.ptyp_attributes <> [] then core_type ctxt f x
    ...
    | Ptyp_constr (li, l) -> (* here, l is the list of argument on l *)
        pp f (* "%a%a@;" *) "%a%a"
          longident_loc li
          (fun f l -> match l with
             |[] -> ()
             |[x]-> pp f " (%a)@;" (core_type1 ctxt) x
             | _ -> list ~first:" (" ~last:")@;" (core_type ctxt) ~sep:") (@;" f l) l
...
and type_def_list ctxt f (rf, l) =
  let type_decl kwd rf f x =
    let eq = ""
       ...
       * else " =" *)
    in
    pp f "@[<2>%s %a%s%a%s%a@]%a" kwd
      nonrec_flag rf (* Format.formatter -> Asttypes.rec_flag -> unit *)
      (x.ptype_name.txt ^ "")
...
and type_declaration ctxt f x =
  ...
  in
  let manifest f =
    match (* x.ptype_manifest *) None with
    | None -> ()
    | Some y ->
